...
export class ChildProcess extends EventEmitter {
   ...
   * @ignore
   * @type {import('./worker_threads.js').Worker}
   */
  get worker() {
    return this.#worker;
  }
...
export function execSync(command, options) {
  ...
  if (result.err) {
    ...
    const message =
      code === "ETIMEDOUT" ? "execSync ETIMEDOUT" : stderr.join("\n");
...
exec[Symbol.for("nodejs.util.promisify.custom")] = exec[
  Symbol.for("socket.runtime.util.promisify.custom")
